...
for(i in 1:nvar){
  ...
  univariate.e.fit <- cfa(univariate.e.mod, data = data, group = "zygosity", orthogonal = TRUE, std.lv = TRUE, missing = "direct")   
   ...
   # funkcija za ekstrakciju parametara univariate
    ...
    colnames(comp.sub) <- c("bic")
    
    rownames(comp.sub) <- c("univariate.ACE", "univariate.AE", "univariate.ADE", "univariate.DE", "univariate.CE", "univariate.E")
    
    comp.sub.dat <- as.data.frame(comp.sub, row.names = c("univariate.ACE", "univariate.AE", "univariate.ADE", "univariate.DE", "univariate.CE", "univariate.E"), colnames = c("bic"))
    ...
    comp.sub.sort <- as.data.frame(csdatsve[order(csdat$bic),])
