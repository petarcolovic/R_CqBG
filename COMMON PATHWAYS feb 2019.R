# common pathways models with specified factor loadings 

# This is a "constrained" (which is an illustrative, but technically incorrect term) variation of common pathways models, where the structure of common factors is specified prior to the analysis. Only the specification of loadings of factors on observed variables is available at this moment. This script does not include the possibility to fix or constrain the loadings, just to attribute them to factors.

# 

# arguments: for the most part, same as in other functions, with an important differences: 

# specification of common factor loadings: specification should include variable names without the extension which marks twin1 and twin2, for example:

# model.fit <- cspecif(data = twindata, first = c("bis_1", "bas_1", "fight_1", "flight_1", "freeze_1"), second = c("bis_2", "bas_2", "fight_2", "flight_2", "freeze_2"), zygo = "zygo", nfaccom = 2, f1 = c("bas", "fight", "flight"), f2 = c("bis", "flight", "freeze"))  
...
# parameter calculation
  ...
  comparison <- as.matrix(anova(cspecif.ace.fit, cspecif.ae.fit, cspecif.ade.fit, cspecif.de.fit, cspecif.ce.fit, cspecif.e.fit))
